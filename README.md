...
- You query profiles based on set critera, aka: all every bouncer who works saturdays within 10 km of location X. Currently the critera are pretty limited but can easily be expanded upon, for example, instead of just available saturday, it might be saturday mornings/evenings/nights. 
...
- You can send a request message to chat, and chat further if it's accepted. from there you can send a "contract" request between a bouncer and club (to be accepted/denied etc). The contract is actually more akin to a friend request, there is no legal rammification to it, and can be deleted at any time by eiter party. 

# Contracts and the Rota
...
The basic flow:
Get all clubs => 
  filter clubs that are require staff on day X, and shuffle them randomly => 
    for club at i, get all bouncers who have contracts with them, not already assigned to a shift that day =>
...
Further, clubs can have contracts between them that allow a bouncer to be condidered for that shift as well (this is an option that can be opted out of):
 *node*        *relationship*
...
I'll go into some (in the appendix ofcourse) more detail about this in my actual write up, but basically, clubs source bouncers through multiple agencies, because of the massive increase in demand for security in the past few years, the bar is effectivly on the floor. If you apply on indeed and have a valid SIA licience, you will be offered a shift by the end of the day.

Obviously the problem with this is that, as agencies are phiscally motivated to fill shifts, you get some wrongungs. 

The way in which my system tries to solve this is quite subtle. I'm mean it doesn't directly solve anything, rather tries it encourages behaviour that leads to the problem being solved.

# To sum up the problem
Clubs are continuely short of bouncers, so they accept anything agencies throw at them, which contributes to the churn of bouncers at most clubs.

The solution the app is trying to promote invloves clubs who geographically grouped to basically share security on one large rota, and on shifts coordinate as such.

I was a part of a system like this when I worked in Bath, where one agency pretty much had a monopoly on the centre (and were actually decent guys), we were assigned a club, but had a radio channel open, hell, there were other clubs accross the street with our guys. We had protocals to converge on a place if it was needed. And guys to who scout the town warning us about "tall guy, arsenel shirt, bald. he was doing coke in the alley, said he was coming to his missus", and other serious threats.

With this system a club doesn't have to stress about not having enough staff on occasion, because worst case scenario is help will arrive in a few minutes. thus the bar doesn't have to be quite so low, and turnover might slow.
